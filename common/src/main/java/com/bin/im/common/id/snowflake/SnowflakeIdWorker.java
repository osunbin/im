...
public class SnowflakeIdWorker implements UidGenerator {
    ...
    public SnowflakeIdWorker(long datacenterId,WorkerIdAssigner assigner) {
        this.datacenterId = datacenterId;
        workerId = assigner.assignWorkerId();
    }

    public SnowflakeIdWorker(WorkerIdAssigner assigner) {
        this.datacenterId = getMachineNum(31);
        workerId = assigner.assignWorkerId();
    }
     ...
     * @功能描述 <pre>反解析UID</pre>
     */
    public String parserUID(Long uid) {
        // 总位数
        long totalBits = 64L;
        // 标识
        long signBits = 1L;
    ...
    public static long getMachineNum(long maxId) {
        byte[] mac = IPConverter.getMachineNum();
        long id = 0L;
        if (mac == null) {
            id = 1L;
        } else {
